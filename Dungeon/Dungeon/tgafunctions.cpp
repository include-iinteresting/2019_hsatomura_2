...
HRESULT CreateTextureFromTgaFile(ID3D11DeviceContext *pContext, const TCHAR *pFilename, ID3D11Texture2D **ppTexture, DWORD *pSrcWidth, DWORD *pSrcHeight, FillMode dwFillmode) {
	...
	hr = DecodeTgaHeader(header, &pixelbits, &bRLE, &bBW, &srcwidth, &srcheight, &srcDescriptor);
	if (FAILED(hr)) {
		goto ERROR_EXIT;
	}

	numElements = srcwidth * srcheight;
	pBuffer = new DWORD[numElements];
	rowPitch = srcwidth * sizeof(DWORD);

	//  convert pixels
	hr = ConvertPixelsFromTgaFile(fp, pBuffer, numElements, bRLE, bBW, pixelbits);
	if (FAILED(hr))
		goto ERROR_EXIT;
	{
		//  create an texture
		...
		//  copy bits
		hr = CopyTgaSourceToTexture2D(pContext,pOutput,pBuffer,srcwidth,srcheight,rowPitch, srcDescriptor,dwFillmode);
		if (FAILED(hr))
			goto ERROR_EXIT;
...
HRESULT CreateD2D1BitmapFromTgaFile(ID2D1RenderTarget *pRenderTarget,const TCHAR *pFilename, ID2D1Bitmap **ppBitmap){
	...
	if (fp != NULL) {
		...
		//	flip x or y
		BYTE flag = srcDescriptor ^ 0x20;
		if (flag & 0x30) {
			DWORD *pBuffer2 = new DWORD[numElements];
			BYTE *pSource;
			BYTE *pDest;
			int diffX = 1, diffY = 1;
			int sourceX = 0, sourceY = 0;
			if (srcDescriptor & 0x10) {
				diffX = -1;						//  flip horizontally
				sourceX = srcwidth - 1;
			}
			if (0 == (srcDescriptor & 0x20)) {
				sourceY = srcheight - 1;		//  flip upside down
				diffY = -1;
			}
			double a, frac = 1.0 / 255.0;
			for (int y = 0; y < (int)srcheight; ++y) {
				pSource = (((BYTE*)pBuffer) + sourceY * rowPitch);
				pSource += sourceX << 2;
				pDest = (((BYTE*)pBuffer2) + y * rowPitch);
		...
		hr = pRenderTarget->CreateBitmap(bitmapSize, pBuffer, rowPitch, &bitmapProperties, &pOutput);
